@extends('layouts.web', ['title' => $category->site_title ?? 'Current Affairs - Prottoy Academy'])
...
<div class="section-premium">
    <div class="container">
        ...
        <div class="premium-breadcrumb">
            ...
            <span>/</span>
            <span class="active">কারেন্ট অ্যাফেয়ার্স (Current Affairs)</span>
        </div>

        <!-- HEADER SUBTITLE -->
        <div class="page-header-premium mb-4">
            <p class="page-subtitle-premium">সহজে ক্যাটাগরি, সাব-ক্যাটাগরি এবং অধ্যায় ভিত্তিক কারেন্ট অ্যাফেয়ার্স অনুসন্ধান করুন</p>
        </div>
        ...
        <div class="stats-grid-academy">
            ...
            <div class="stat-card-academy">
                ...
                <div class="stat-growth-academy"><i class="ri-layout-grid-line"></i> <span id="stat-classes">{{ strtr($stats['classes_count'], ['0'=>'০','1'=>'১','2'=>'২','3'=>'৩','4'=>'৪','5'=>'৫','6'=>'৬','7'=>'৭','8'=>'৮','9'=>'৯']) }}</span>টি ক্যাটাগরি</div>
            </div>
        </div>
        ...
        <div class="academy-layout">
            ...
            <div class="academy-sidebar">
                ...
                <!-- Class Filters (Main Categories) -->
                <div class="filter-group-academy">
                    <h4 class="filter-title-academy">ক্যাটাগরি</h4>
                ...
                <!-- Subject Filters (Job Categories) -->
                <div class="filter-group-academy" id="subject-filter-group" style="display: none;">
                    <h4 class="filter-title-academy">সাব-ক্যাটাগরি</h4>
                    <div class="filter-list-academy">
                        @foreach ($stats['subjects'] as $sub)
                            <div class="subject-item-wrapper-academy" data-class-ids="{{ implode(',', $sub['class_ids']) }}" style="display: none;">
...
<script>
    ...
    function updateSubjectsVisibility() {
        ...
        document.querySelectorAll(".subject-item-wrapper-academy").forEach(wrapper => {
            const classIdsStr = wrapper.getAttribute('data-class-ids') || '';
            const classIds = classIdsStr.split(',');
            const isVisible = checkedClassIds.some(id => classIds.includes(id));
    ...
    function applyFilters(page = 1) {
        ...
        if (classIds.length === 0 || subjectIds.length === 0 || types.length === 0) {
            ...
            let missing = [];
            if (classIds.length === 0) missing.push('ক্যাটাগরি');
            if (subjectIds.length === 0) missing.push('সাব-ক্যাটাগরি');
        ...
        fetch("{{ route('current-affairs.filter') }}?" + params.toString())
            .then(res => res.json())
            .then(data => {
                console.log("Current Affairs filter AJAX response data:", data);
                document.getElementById('academy-loader').style.display = 'none';
    ...
    function updatePrintTemplate(questions) {
        try {
            ...
            const classDetail = classNames.length > 0 ? classNames.join(', ') : 'সব ক্যাটাগরি';
            const subjectDetail = subjectNames.length > 0 ? subjectNames.join(', ') : 'সব সাব-ক্যাটাগরি';
            const chapterDetail = chapterNames.length > 0 ? chapterNames.join(', ') : 'সব টপিক';
            ...
            for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
                ...
                watermark.innerText = 'प्रत्यয় একাডেমি';
                ...
                headerTitle.innerText = 'কারেন্ট অ্যাফেয়ার্স';
                
                const headerMeta = document.createElement('div');
                headerMeta.className = 'print-header-meta';
                headerMeta.innerHTML = `
                    <div><strong>ক্যাটাগরি:</strong> ${classDetail} | <strong>সাব-ক্যাটাগরি:</strong> ${subjectDetail}</div>
                    <div><strong>টপিক:</strong> ${chapterDetail} | <strong>পৃষ্ঠা:</strong> ${formatBanglaNumber(pageIdx + 1)}/ ${formatBanglaNumber(totalPages)}</div>
                `;
    ...
    function printAllQuestions() {
        // Gather Class IDs (Categories)
        const classIds = [];
        document.querySelectorAll("input[name='class_ids[]']:checked").forEach(checkbox => {
            classIds.push(checkbox.value);
        });

        // Gather Subject IDs (Sub-categories)
        const subjectIds = [];
        document.querySelectorAll("input[name='subject_ids[]']:checked").forEach(checkbox => {
            subjectIds.push(checkbox.value);
        });

        // Gather Chapter IDs (Topics)
        const chapterIds = [];
        document.querySelectorAll("input[name='chapter_ids[]']:checked").forEach(checkbox => {
            chapterIds.push(checkbox.value);
        });
        ...
        // If selection is incomplete (must check at least one Class, one Subject, and one Type)
        if (classIds.length === 0 || subjectIds.length === 0 || types.length === 0) {
            toastr.warning("অনুগ্রহ করে কমপক্ষে একটি ক্যাটাগরি, একটি সাব-ক্যাটাগরি এবং একটি প্রশ্নের ধরন নির্বাচন করুন।");
            return;
        }
