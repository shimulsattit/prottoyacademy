<form action="{{ route("portal.featured-banners.update", $model->id) }}" method="POST" class="ajax-form">
    @method('PATCH')
    ...
    <div class="modal-body">
        <div class="row">
            ...
            <div class="col-md-12 form-group">
                ...
                <select name="category_id" id="category_id" class="form-control select" required data-placeholder="Select Category" data-parsley-errors-container="#category_id_error">
                    ...
                    @foreach ($categories as $category)
                        <option {{ $model->category_id == $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->category->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-12 mt-3 form-group">
                ...
                <input type="file" name="banner" data-default-file="{{ $model->banner ? asset($model->banner) : '' }}" id="banner" class="form-control dropify">
            </div>

            <div class="col-md-6 form-group mt-3">
                <label for="alt_tag">Alt Tag</label>
                <input type="text" name="alt_tag" id="alt_tag" class="form-control required" value="{{ $model->alt_tag }}">
            </div>
            
            <div class="col-md-6 form-group mt-3">
                <label for="content">Short Content</label>
                <input type="text" name="content" id="content" class="form-control required" value="{{ $model->content }}">
            </div>
