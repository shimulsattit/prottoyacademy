@extends('layouts.admin', ['title' => 'শিক্ষার্থী ব্যবস্থাপনা'])
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold">🎓 শিক্ষার্থী ব্যবস্থাপনা</h4>
            <p class="text-muted mb-0">সকল নিবন্ধিত শিক্ষার্থীর তালিকা ও ব্যবস্থাপনা</p>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
            ...
            <div class="card border-0 shadow-sm" style="border-radius:14px; background: linear-gradient(135deg,#1B4F72,#2E86C1);">
                <div class="card-body text-white p-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div><div class="fs-3 fw-bold">{{ $totalStudents }}</div><div class="small opacity-75">মোট শিক্ষার্থী</div></div>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm" style="border-radius:14px; background: linear-gradient(135deg,#1E8449,#27AE60);">
                <div class="card-body text-white p-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div><div class="fs-3 fw-bold">{{ $activeStudents }}</div><div class="small opacity-75">সক্রিয়</div></div>
                        <div style="font-size:36px; opacity:.3;">✅</div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm" style="border-radius:14px; background: linear-gradient(135deg,#E74C3C,#C0392B);">
                <div class="card-body text-white p-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div><div class="fs-3 fw-bold">{{ $blockedStudents }}</div><div class="small opacity-75">ব্লকড</div></div>
                        <div style="font-size:36px; opacity:.3;">🚫</div>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-hover mb-0">
                    <thead style="background:#f8fafc;">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-muted small fw-bold">ফোন</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="px-4 py-3 text-muted small">{{ $students->firstItem() + $i }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-muted">{{ $s->phone ?? '—' }}</td>
                            <td class="px-4 py-3">
                                @if($s->status == 1)
                            ...
                            <td class="px-4 py-3 text-muted small">{{ $s->created_at->format('d M Y') }}</td>
                            <td class="px-4 py-3">
                                <div class="d-flex gap-1 flex-wrap">
                                    @if($s->status == 1)
                                    <form action="{{ route('portal.students.block', $s->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button class="btn btn-warning btn-sm rounded-pill px-3">🚫 ব্লক</button>
                                    </form>
                                    @else
                                    <form action="{{ route('portal.students.unblock', $s->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button class="btn btn-primary btn-sm rounded-pill px-3">🔓 আনব্লক</button>
                                    </form>
                                    @endif
                                    <form action="{{ route('portal.students.destroy', $s->id) }}" method="POST" class="d-inline" onsubmit="return confirm('শিক্ষার্থী মুছে দিতে চান?')">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                        <tr><td colspan="7" class="text-center py-5 text-muted">কোনো শিক্ষার্থী পাওয়া যায়নি।</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
