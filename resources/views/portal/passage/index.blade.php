@extends('layouts.admin', ['title' => 'Passage Management'])
@push('style')
            ...
            <div id="app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">Passage Management</li>
                    </ul>
                </div>

                @if(Auth::guard('admin')->user()->hasPermissionTo('passage.create'))
                    <div class="d-flex align-items-center gap-2 gap-lg-3">
                        <a href="{{ route('portal.passage.create') }}" class="btn btn-sm fw-bold btn-primary">
                            Create
                        </a>
                    </div>
                @endif
    ...
    <script>
        $(function () {
            var table = $('#data-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('portal.passage.index') }}",
                columns: [
                    {
                        data: 'id',
                        name: 'id',
