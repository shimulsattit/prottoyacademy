...
class FeaturedCategoryController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($models)
                ...
                ->addColumn('action', function ($model) {
                    return view('portal.featured-category.action', compact('model'));
                })
                ->editColumn('created_at', function($model) {
                    return date('d F, Y h:i A', strtotime($model->created_at));
                })
                ->rawColumns(['action', 'category', 'url', 'status', 'created_at'])
                ->make(true);
        }

        return view('portal.featured-category.index');
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('portal.featured-category.create', compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return response()->json([
            ...
            'goto' => route('portal.featured-categories.index')
        ]);
    }
    ...
    {
        return FeaturedCategory::find($id);
    }
    ...
    public function edit(string $id)
    {
        ...
        $categories = Category::whereNull('parent_id')->where('status', 1)->get();
        return view('portal.featured-category.edit', compact('model', 'categories'));
    }
    ...
    {
        $model = FeaturedCategory::findOrFail($id);
        if(!$model) {
            return response()->json([
                'status' => false, 
